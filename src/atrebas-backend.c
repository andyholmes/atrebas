                              ...
                              GError       **error)
{
  ...
  name = json_object_get_string_member_with_default (properties,
                                                     "Name",
                                                     "Unknown");
  name_fr = json_object_get_string_member_with_default (properties,
                                                        "FrenchName",
                                                        name);
  uri = json_object_get_string_member_with_default (properties,
                                                    "description",
                                                    "https://native-land.ca");
  uri_fr = json_object_get_string_member_with_default (properties,
                                                       "FrenchDescription",
                                                       uri);
  color = json_object_get_string_member_with_default (properties,
                                                      "color",
                                                      "#000000");
  slug = json_object_get_string_member_with_default (properties,
                                                     "Slug",
                                                     "");
  ...
  for (unsigned int i = 0; i < n_features; i++)
    {
      ...
      properties = json_object_get_object_member (feature, "properties");
      json_object_set_int_member (properties, "theme", theme);
  ...
  for (unsigned int i = 0; i < G_N_ELEMENTS (local_sources); i++)
    {
      ...
      if ((stream = g_file_read (file, cancellable, &error)) != NULL)
        input = G_INPUT_STREAM (stream);

      if (error != NULL)
        return g_task_return_error (task, error);

      if (!json_parser_load_from_stream (parser, input, cancellable, &error))
        return g_task_return_error (task, error);
